...
class FilterTreeviewItem extends TreeviewItem {
	...
	updateRefChecked(): void {
		...
		if (refChecked && this.refItem?.children) {
...
export class TreeviewComponent implements OnChanges, OnInit {
	...
	public headerTemplateContext?: TreeviewHeaderTemplateContext;
	public allItem: TreeviewItem;
	public filterText = '';
	public filterItems?: TreeviewItem[];
	public selection?: TreeviewSelection;
	...
	onAllCollapseExpand(): void {
		...
		if (this.filterItems) {
			this.filterItems
				.forEach(item => item.setCollapsedRecursive(this.allItem.collapsed));
		}
	}
	...
	onItemCheckedChange(item: TreeviewItem, _checked: boolean): void {
		if (item instanceof FilterTreeviewItem) {
			item.updateRefChecked();
		}
	...
	private generateSelection(): void {
		...
		this.selection = {checkedItems, uncheckedItems};
	}
	...
	private filterItem(item: TreeviewItem, filterText: string): TreeviewItem | undefined {
		const isMatch = item.text.toLowerCase().indexOf(filterText) >= 0;
		if (isMatch) {
			return item;
		} else {
			if (item.children) {
	...
	private updateCheckedOfAll(): void {
		let itemChecked: boolean | null | undefined = null;
